...
The `.bin` file is the runtime image used by the default XDMA H2C load path.
The `.txt` file is generated by Bin2ddr for the JTAG DDR fallback/debug path.
...
## DDR Randomization, or Image Padding

For Linux workloads, the kernel or initramfs can legally touch physical DDR outside the exact bytes occupied by the image file.
If that area is left uninitialized on FPGA, the load/store can become misaligned with NEMU.

Use one of these two policies:

| Policy | When to use | Command shape |
|--------|-------------|---------------|
